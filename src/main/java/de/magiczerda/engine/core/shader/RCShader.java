...
public abstract class RCShader {

    protected int shaderID = -1;

    public RCShader(ShaderType shaderType, String sourcePath) {
    ...
    public void compile(String sourcePath, ShaderType shaderType) {
        try {
            ...
            GL33.glShaderSource(shaderID, uploadConstants(source));
    ...
    protected String uploadConstants(String shaderSource) {
        String[] replace = insertConstants();
        if(replace == null) return shaderSource;
        if(replace.length != 2) return shaderSource;

        else {
            if(!shaderSource.contains(replace[0])) return shaderSource;
            else return shaderSource.replace(replace[0], replace[1]);
        }
    }


    protected abstract String[] insertConstants();
